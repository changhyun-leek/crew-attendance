...
def build():
    ...
    r = sub.add_run("크루교사 · 임원교사 · 보조교사를 위한 빠른 안내")
    set_font(r, size=13, color=MUTED)
    add_callout(doc, "한 문장으로", "휴대폰에서는 빠르게 출석을 체크하고, 컴퓨터에서는 전체 현황과 보고사항을 한눈에 관리하는 청소년부 공용 시스템입니다.")
    add_para(doc, "초안 검토용 · 2026년 7월", color=MUTED, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)

    page_break(doc)
    add_heading(doc, "1. 시스템 한눈에 보기", 1)
    add_para(doc, "사이트 첫 화면에서 본인의 역할을 선택하면 필요한 기능만 보이도록 구성되어 있습니다. 글자와 버튼을 크게 배치해 휴대폰에 익숙하지 않은 사용자도 순서대로 누르면 사용할 수 있습니다.")
    ...
    add_heading(doc, "처음 접속할 때", 2)
    for text in (
        "첫 화면에서 크루교사, 임원교사, 보조교사 중 하나를 누릅니다.",
        "크루교사와 임원교사는 이름을 누르거나 검색창에서 이름을 찾습니다.",
        "숫자 PIN 4~6자리를 입력합니다. 개인 기기에서는 로그인이 최대 30일 유지됩니다.",
        "공용 기기에서는 사용 후 반드시 로그아웃합니다.",
    ):
        add_list_item(doc, text, number_id)
    add_callout(doc, "보안 안내", "교사 PIN은 브라우저 코드나 일반 데이터 표에 저장되지 않습니다. PIN을 5회 연속 잘못 입력하면 해당 계정은 10분간 잠깁니다.", tone="gold")

    page_break(doc)
    add_heading(doc, "2. 크루교사와 보조교사 출석체크", 1)
    add_heading(doc, "크루교사", 2)
    for text in (
        "출석 날짜와 크루명을 확인합니다.",
        "학생별로 큰 출석 또는 결석 버튼을 누릅니다. 같은 버튼을 다시 누르면 미체크로 돌아갑니다.",
        "결석 이유나 연락 상태가 있으면 학생의 사유·비고 버튼을 누릅니다.",
        "임원이 요청한 임시 항목이 보이면 학생과 확인한 값을 선택합니다.",
        "마지막으로 미체크 인원을 확인하고 보고 문구 복사를 눌러 단톡방 보고에 사용합니다.",
    ):
        add_list_item(doc, text, number_id)
    add_heading(doc, "보조교사", 2)
    add_para(doc, "보조교사는 계정 없이 본인 이름을 입력하고 선택한 크루의 오늘 출석만 체크합니다. 화면 상단에는 현재 보조교사 이름이 항상 표시됩니다.")
    for text in (
        "최근 사용 이름을 고르거나 새 이름을 직접 입력합니다.",
        "오늘 출석·결석, 결석 사유, 연락 상태, 임시 확인 항목만 수정합니다.",
        "이름을 잘못 입력하면 상단 이름을 눌러 수정합니다. 이전 이름과 수정 이름은 변경 이력에 함께 남습니다.",
    ):
        add_list_item(doc, text, bullet_id)
    add_callout(doc, "저장 상태 확인", "저장 중 → 저장됨 표시를 확인합니다. 인터넷 연결 경고나 저장 실패가 보이면 연결이 복구될 때까지 저장 완료로 판단하지 않습니다.", tone="gold")

    page_break(doc)
    add_heading(doc, "3. 사유·비고와 임원 요청사항", 1)
    add_para(doc, "학생 관련 내용은 성격에 따라 구분해 기록하면 임원이 필요한 일을 놓치지 않고, 당일 사유와 장기 확인사항이 섞이지 않습니다.")
    ...
    add_heading(doc, "임원 공지와 임시 칸", 2)
    for text in (
        "임원 화면에서 공지·임시 항목 메뉴를 엽니다.",
        "공지 또는 학생별 임시 확인 칸을 선택합니다.",
        "제목, 설명, 게시 시작일과 종료일을 입력합니다.",
        "임시 칸은 쉼표로 선택지를 적습니다. 예: 신청, 미신청, 고려중.",
        "게시 기간이 끝나면 출석 화면에서는 자동으로 숨겨지고 기존 응답은 보존됩니다.",
    ):
        add_list_item(doc, text, number_id)
    add_callout(doc, "비고 학생 구분", "임원 출석표에서 사유, 연락 안 됨 또는 학생 비고가 있는 학생은 노란색 행과 확인 필요 표식으로 바로 구분됩니다.")

    page_break(doc)
    add_heading(doc, "4. 임원교사 관리 화면", 1)
    add_para(doc, "임원 화면은 컴퓨터 사용을 기준으로 왼쪽 메뉴, 상단 필터, 중앙 데이터 표를 배치했습니다. 태블릿과 휴대폰에서는 메뉴가 접히지만 대량 확인과 내보내기는 컴퓨터 사용을 권장합니다.")
    add_table(doc, ["메뉴", "주요 기능"], [
        ["전체 현황", "크루 수, 학생 수, 출석·결석, 출석률 요약"],
        ["출석 기록", "날짜·크루·학생·상태·비고별 검색과 상세 확인"],
        ["공지·임시 항목", "교사 공지와 기간 한정 학생별 조사 생성"],
        ["오류·개선 의견", "교사 의견을 새 의견 → 확인 중 → 완료로 관리"],
        ["크루·교사·학생", "등록, 배정, PIN 초기화, 상태와 소속 관리"],
        ["변경 이력", "누가 언제 어떤 출석과 정보를 바꿨는지 확인"],
    ], [2700, 6660])
    ...
    page_break(doc)
    add_heading(doc, "5. 오류·개선 의견과 유지보수", 1)
    add_para(doc, "교사 피드백을 바로 모으고 수정 여부를 추적할 수 있도록 모든 로그인 후 화면에 오류·개선 의견 보내기 버튼이 있습니다.")
    for text in (
        "교사가 역할, 화면, 문제 상황과 원하는 개선점을 적어 보냅니다.",
        "임원은 의견함에서 새 의견을 확인하고 확인 중으로 변경합니다.",
        "관리 담당자가 프로젝트 수정과 테스트를 진행합니다.",
        "데모 화면에서 담당 교사가 수정 결과를 확인합니다.",
        "운영 배포 후 임원이 의견 상태를 완료로 변경합니다.",
    ):
        add_list_item(doc, text, number_id)
    add_heading(doc, "수정 요청에 적으면 좋은 내용", 2)
    for text in (
        "사용 역할과 화면 이름",
        "어떤 버튼을 어떤 순서로 눌렀는지",
        "기대한 결과와 실제 결과",
        ...
        add_list_item(doc, text, bullet_id)
    add_callout(doc, "직접 확인하는 가장 쉬운 방법", "프로젝트 폴더의 개발_미리보기_실행.bat을 더블클릭합니다. 데모 PIN은 1234이며 실제 출석 데이터는 바뀌지 않습니다.")

    page_break(doc)
    add_heading(doc, "6. 문제 발생 시와 안전 원칙", 1)
    ...
    add_heading(doc, "개인정보와 기록 보존", 2)
    for text in (
        "학생 정보는 사역에 꼭 필요한 범위에서만 기록합니다.",
        "PIN, 서비스 권한 키, 초기 관리자 코드는 문서·단톡방·화면 캡처에 적지 않습니다.",
        "학생은 삭제하지 않고 장기결석 또는 퇴실로 전환해 과거 출석을 보존합니다.",
        "운영 변경 전에는 기존 출석을 백업하고 이관 전후 건수를 비교합니다.",
        "공용 컴퓨터와 공용 휴대폰에서는 반드시 로그아웃합니다.",
    ):
        add_list_item(doc, text, bullet_id)
    add_callout(doc, "검토용 초안", "이 문서는 담당 교역자와 임원 검토를 위한 초안입니다. 교사 이름, 크루 배정, 초기 PIN, 운영 시작일은 실제 도입 전에 임원 협의로 확정합니다.", tone="gold")
