...
def build():
    ...
    add_callout(doc, "가장 쉬운 시작", "운영 사이트를 연 뒤 화면 왼쪽 아래의 앱 설치를 누릅니다. 설치가 끝나면 휴대폰 홈 화면이나 컴퓨터 바탕 화면·시작 메뉴의 새벽이슬 아이콘으로 바로 실행할 수 있습니다.")
    ...
    add_table(doc, ["기기", "설치 순서"], [
        ["안드로이드", "Chrome 또는 Samsung Internet에서 앱 설치를 누릅니다. 자동 버튼이 없으면 브라우저 메뉴의 앱 설치 또는 홈 화면에 추가를 선택합니다."],
        ["아이폰·아이패드", "Safari에서 공유를 누른 뒤 홈 화면에 추가, 추가 순서로 누릅니다."],
        ["Windows·Mac", "Chrome 또는 Edge 주소창의 설치 아이콘을 누릅니다. 아이콘이 없으면 브라우저 메뉴에서 앱 설치를 선택합니다."],
    ], [2160, 7200])
    add_callout(doc, "카카오톡에서 링크를 연 경우", "카카오톡 안에서는 설치가 제한될 수 있습니다. 우측 상단 메뉴에서 다른 브라우저로 열기를 선택한 뒤 위 순서대로 설치합니다.", tone="gold")
    add_heading(doc, "설치 후 기억할 점", 2)
    for text in (
        "설치해도 교사 권한과 PIN 보안은 그대로 유지됩니다.",
        "개인 기기에서는 자동 로그인을 사용하고, 다른 교사가 같은 기기를 쓸 때는 먼저 로그아웃합니다.",
        "새 버전 알림이 나타나면 출석 입력을 마친 뒤 업데이트를 누릅니다.",
        "인터넷이 끊기면 화면은 열릴 수 있어도 출석 저장은 되지 않으므로 연결 경고를 확인합니다.",
        "화면 모드 선택은 기기에 저장되며, 휴대폰은 보라색 십자가·체크, 컴퓨터는 모니터 아이콘으로 구분합니다.",
    ):
    ...
    for text in (
        ...
        add_list_item(doc, text, number_id)
    add_callout(doc, "저장·PIN·휴대폰 알림", "학생별 표시가 저장됨으로 바뀌었는지 확인합니다. 출석 화면 아래에서 현재 PIN 확인 후 내 PIN을 바꿀 수 있고, 알림 켜기를 누르면 임원의 독려 알림을 받을 수 있습니다.", tone="gold")
    ...
    for text in (
        ...
        "조회를 눌러 표를 갱신합니다.",
        "크루별 접힌 제목에서 출석·결석·미체크 인원과 출석률을 보고 필요한 크루만 펼칩니다.",
        "노란색 행과 확인 필요 표식이 있는 학생의 사유·연락·비고를 확인합니다. 출석 학생의 결석 사유·연락 칸은 비워 표시됩니다.",
    ):
        add_list_item(doc, text, number_id)
    add_callout(doc, "확인 필요 학생", "결석 사유, 연락 안 됨 또는 학생 비고가 있는 학생은 표에서 노란색으로 표시되어 먼저 확인할 수 있습니다.")
    add_callout(doc, "출석 독려 알림", "전체 현황에서 아직 완료하지 않은 크루의 독려 알림 보내기를 누릅니다. 교사가 먼저 휴대폰 알림을 켜야 하며 같은 크루에는 30분에 한 번만 보낼 수 있습니다.")
